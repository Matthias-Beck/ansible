...
short_description: manage E-Series LDAP integration for authentication
description:
...
class Ldap(object):
    ...
    def clear_single_configuration(self):
        configuration = self.get_configuration(self.identifier)
        updated = False
        msg = self.NO_CHANGE_MSG
        if configuration:
            updated = True
            msg = "The LDAP domain configuration for [%s] was cleared." % self.identifier
            if not self.check_mode:
                try:
                    (rc, result) = request(self.url + self.base_path + '%s' % (self.identifier),
                                           method='DELETE', **self.creds)
                except Exception as err:
    ...
    def clear_configuration(self):
        ...
        if configuration['ldapDomains']:
            ...
            if not self.check_mode:
                try:
                    (rc, result) = request(self.url + self.base_path, method='DELETE', **self.creds)
                except Exception as err:
